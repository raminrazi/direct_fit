...
if(ODE_str == 'fitzhugh'):
    ...
    d = 5 # set the number of states
    x0 =  np.random.normal(0,0.001,(d,))
    x0 = x0.tolist()
    X, Y, dt = simulate(ODE_str, x0, true_param=[8], end_t=2.6, dt=.01, noise_var=.5)
    init_param = np.array([1000])

# learn the parameters, estimated states, and predicted states
params,est_X, pred_X = fit_direct(Y,dt,init_param,ODE_str,lam=1,max_iters=100000,tol=1e-8)
